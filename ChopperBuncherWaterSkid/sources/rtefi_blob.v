...
// hand-edit it!
//
// This Verilog include file was machine-generated by /a/devuser/latshaw/Desktop/Berkley/lcls2_llrf-master/firmware/submodules/bedrock//badger/collect_clients.py
// 1 /a/devuser/latshaw/Desktop/Berkley/lcls2_llrf-master/firmware/submodules/bedrock//badger/hello.v
// 2 /a/devuser/latshaw/Desktop/Berkley/lcls2_llrf-master/firmware/submodules/bedrock//badger/speed_test.v
// 3 /a/devuser/latshaw/Desktop/Berkley/lcls2_llrf-master/firmware/submodules/bedrock//badger/mem_gateway.v
// filling in unused UDP ports starting at 4
...
module rtefi_blob(
	...
	input  p2_nomangle,
	output  [23:0] p3_addr,
	output  p3_control_strobe,
	output  p3_control_rd,
	output  p3_control_write,
	output  p3_control_rd_valid,
	output  [p3_read_pipe_len:0] p3_control_pipe_rd,
	output  [31:0] p3_data_out,
	input  [31:0] p3_data_in,
...
parameter p3_read_pipe_len = 3;
parameter p3_enable_bursts = 0;
...
speed_test #(.n_lat(n_lat)) p2_client(.clk(clk), .nomangle(p2_nomangle), .len_c(len_c), .idata(idata), .raw_l(raw_l[1]), .raw_s(raw_s[1]), .odata(odata_p2));
mem_gateway #(.n_lat(n_lat), .read_pipe_len(p3_read_pipe_len), .enable_bursts(p3_enable_bursts)) p3_client(.clk(clk), .len_c(len_c), .idata(idata), .raw_l(raw_l[2]), .raw_s(raw_s[2]), .odata(odata_p3), .addr(p3_addr), .control_strobe(p3_control_strobe), .control_rd(p3_control_rd), .control_write(p3_control_write), .control_rd_valid(p3_control_rd_valid), .control_pipe_rd(p3_control_pipe_rd), .data_out(p3_data_out), .data_in(p3_data_in));
