...
ENTITY led_i2c IS
	...
	(	
		...
		SDA 		 : INOUT STD_LOGIC; -- data line (data is bi directional), changed for xilinx 3/24/22
...
ARCHITECTURE rtl OF led_i2c IS 
	...
	attribute dont_touch of SCL_Buffer, SCL_advanced, en_pulse, SDA_IN, SDA_buffer, SCL_HOLD, packet, oe_reads, ack_expected  : signal is "true";
...
--
	...
	gen_counter_normal : if testBench = '0' generate
	   -- upodated on again 3/23/22  to get a 50 kHz rate. 
	   -- for xilinx board.
		-- real count values to support I2C
		count_FULL	<= x"09C4"; --again 3/23/22 for xilinx,  JAL updated 1/26/22 was 04E2
		count_Half	<= x"0271"; -- 
		count_adv	<= x"0138"; -- 
...
--=================================================
	...
	begin
		if reset_n = '0' then
			...
			if (SCL_Buffer = '1' and SCL_Buffer'event) then
				...
				IF ack_expected = '1' THEN S_ACK  <= SDA_IN; -- ack bits should be low, changed to SDA_IN 3/24/22
				ELSE								S_ACK  <= '1';
				END IF;
			end if;
		end if;
   ...
   generic map (
      ...
      SLEW => "SLOW")
   port map (
      O =>  SDA_IN, -- Buffer output (to FPGA)
      IO => SDA,        -- Buffer inout port (connect directly to top-level port)
      I =>  SDA_BUFFER,     -- Buffer input (from FPGA)
      T =>  oe_reads    -- 3-state enable input, high=input, low=output 
   );
	--
