...
// JAL, manually set this IP
//rfdaqnl02 - 129.57.202.68
//rfdaqnl03 - 129.57.202.69
...
//parameter [31:0] ip = {8'd192, 8'd168, 8'd50, 8'd1};  // 192.168.50.1 is lab testing IP mode
//parameter [47:0] mac = 48'h125555000145; // make last to values of MAC be the same as IP address (note IP is decimal and MAC is HEX)
...
// JAL removeed because we are not using the demo
...
rtefi_blob #(.mac_aw(tx_mac_aw), .p3_enable_bursts(enable_bursts)) rtefi(
    .ip(ip), .mac(mac),
	...
	.p3_data_out(lb_data_out), .p3_data_in(lb_rdata), // JAL, changed lb_rdata to be lb_rdata
...
//JAl new outputs to make this as similar to other modules
assign lb_clk_out = tx_clk;
assign lb_addr_out = lb_addr;
assign lb_valid = lb_control_strobe;
assign lb_rnw = lb_control_rd;
assign lb_wdata = lb_data_out;
