...
// JAL debug, changed IP for jlab dev
//parameter [31:0] ip = {8'd192, 8'd168, 8'd50, 8'd1};  // 192.168.7.4
...
reg [7:0] ip_mem[0:15];
//always @(posedge config_clk) if (config_s) ip_mem[config_a] <= config_d;   // JAL, 4/25/2025 I don't use SPI_GATE and this is giving a multi driver errror. maybe by changing the init i broke it...
always @(posedge rx_clk) ip_d <= ip_mem[ip_a];
always @(posedge tx_clk) ip_mem_d_tx <= ip_mem[ip_mem_a_tx];
//initial begin / JAL, changed this to an always so we can update IP/MAC dynamically
// also changed '=' to '<=' to support register assignments in the always statment
always @(posedge rx_clk) begin
